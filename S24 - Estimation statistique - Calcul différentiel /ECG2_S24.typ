...
+ Deuxième méthode.
  ...
  + On pose $A_n = m/n X_n$. Montrer que $A_n$ est un estimateur sans biais et convergent de $N$, et déterminer son risque quadratique.
]
...
+ Deuxième méthode.
  ...
  + On a
    ...
    Enfin, son risque quadratique vaut
    $
    E((A_n - N)^2) = V(A_n) = (m/n)^2 V(X_n) = N(N - m)/n.
    $
]
...
+ On a déjà
  ...
  Et le risque quadratique de $T_n$ en $a$ vaut
  $
  r(T_n) = b(T_n)^2 + V(T_n) = a^2/((n - 1)^2) + n^2 a^2/((n - 1)^2 (n - 2)) = a^2 (n^2 + n - 2)/((n - 1)^2 (n - 2)) ->_(n -> +infinity) 0.
  $
  Puisque le risque quadratique tend vers $0$, on en déduit que $T_n$ est un estimateur convergent de $a$.
]
