...
#exercice(title: "Spectre clair", n_stars:1)[
 Soit $E$ un $RR$-espace vectoriel de dimension finie. #linebreak() Soient $F,G$ des sous espaces vectoriels de $E$ tels que $F dsum G = E$. On note $p$ le projecteur sur $F$ parallèlement à $G$.
  #linebreak() Déterminer les valeurs propres de $p$ et les vecteurs propres associés.
]

#correction[
Soient $lambda in RR$ et $x in E$ tel que $p(x) = lambda x.$ Comme $F dsum G = E$, on peut écrire $x = x_F + x_G$ avec $x_F in F$ et $ x_G in G$. Alors, on a $p(x) = x_F = lambda (x_F + x_G) <=> (1 - lambda)x_F = lambda x_G.$
  ...
  Cas 3 : $lambda in.not {0,1}$. #linebreak()
  Alors, on a égalité entre un élément de $F$ et un élément de $G$. Or, $F inter G = {0}$ donc $x_F = x_G = 0$, donc $x = 0$. Ainsi, $lambda$ n'est pas valeur propre de $p$.
